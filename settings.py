...
class Settings:
  # 1. Configuración de rutas
    ...
    CLIMATE_DATA_PATH:Final[str] = str(MAIN_CLIMATE_PATH/"datos_clima")
    CONFIG_APIS_FILE_PATH:Final[str] = str(MAIN_CLIMATE_PATH/"datos_clima"/"CLIMATE_APIS_CONFIG.json")
    METEOGAL_API:Final[str] = environ['METEOGALICIA_API_KEY']
    # PLC
    VM_PATH:Final[str] = str(MAIN_PATH/"backend"/"PLC"/"config"/"virtualMemories.json") 
    ZM_PATH:Final[str] = str(MAIN_PATH/"backend"/"PLC"/"config"/"zonesMemories.json")
    TZ_PATH:Final[str] = str(MAIN_PATH/"backend"/"PLC"/"config"/"timeZonesData.json") 
    SSM_PATH:Final[str] = str(MAIN_PATH/"backend"/"PLC"/"config"/"statusSystemMemories.json")

  # 2. Configuración del PLC
    MEMORIE_BYTES_READ:Final[int] = 5 #Bytes de memoria a leer
    IP_LOGO:Final[str] = "192.168.1.252" #"192.168.2.252"
